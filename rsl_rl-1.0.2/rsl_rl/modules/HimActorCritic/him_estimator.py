...
class HIMEstimator(nn.Module):
    def __init__(self,
        ...
        # Encoder
        enc_input_dim = int(self.temporal_steps * self.num_one_step_obs)
        enc_layers = []
    ...
    def update(self, obs_history, next_input_obs, prev_critic_obs, lr=None):
        if lr is not None:
            self.learning_rate = lr
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = self.learning_rate
        
        vel = prev_critic_obs[:,73:76]
        next_obs = next_input_obs.detach()
