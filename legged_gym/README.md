...
### CODE STRUCTURE ###
1. Each task is defined by its own env file and config file, for example `go2w/go2w_robot.py` + `go2w/go2w_config.py` and `nezha/nezha_robot.py` + `nezha/nezha_config.py`.
2. Task envs inherit the thin `ManagerBasedTask` lifecycle base, while task configs inherit only `BaseConfig` and define their own env, asset, control, reward, action, observation, termination, and event sections.
3. Reward, observation, action, termination, and event behavior is configured through `ManagerTermCfg` entries. Legacy `cfg.rewards.scales` reward discovery is still available for compatibility.
4. Tasks must be registered using `task_registry.register(name, EnvClass, EnvConfig, TrainConfig)`. This is done in `envs/__init__.py`, but can also be done from outside of this repository.  

### Manager Architecture ###
This fork now uses an IsaacLab-inspired manager layer under `legged_gym/managers`.
`ManagerBasedTask` owns the shared RL lifecycle, while `ActionManager`, `CommandManager`, `ObservationManager`, `RewardManager`, `TerminationManager`, and `EventManager` dispatch task behavior. `legged_robot.py` and `legged_robot_config.py` are kept only as thin compatibility aliases; new tasks should inherit `ManagerBasedTask` and `BaseConfig` directly.
