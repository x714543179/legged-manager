...
class Go2w(ManagerBasedTask):
    ...
    def _update_terrain_curriculum(self, env_ids):
        mdp.terrain_levels(self, env_ids)
    
    def update_command_curriculum(self, env_ids):
        mdp.command_ranges(self, env_ids)


    def _get_noise_scale_vec(self, cfg):
        """Build observation noise scales from observation manager terms."""
        self.add_noise = self.cfg.noise.add_noise
        from legged_gym.managers import ObservationManager

        observation_manager = ObservationManager(self, getattr(cfg, "observations", None))
        manager_noise_vec = observation_manager.compute_noise_scale_vec()
        if manager_noise_vec is not None:
            if manager_noise_vec.numel() != self.num_obs:
                raise ValueError(
                    f"Observation noise vector has {manager_noise_vec.numel()} entries, "
                    f"but cfg.env.num_observations is {self.num_obs}."
                )
            return manager_noise_vec
    ...
    def _init_buffers(self):
        ...
        self.cmd_action_latency_buffer = torch.zeros(
            self.num_envs,
            self.num_actions,
            mdp.latency_buffer_length(self, "actions", "command_latency", default=(0, 0)),
            device=self.device,
        )
        self.obs_motor_latency_buffer = torch.zeros(
            self.num_envs,
            self.num_actions * 2,
            mdp.latency_buffer_length(self, "observations", "motor", default=(0, 0)),
            device=self.device,
        )
        self.obs_imu_latency_buffer = torch.zeros(
            self.num_envs,
            6,
            mdp.latency_buffer_length(self, "observations", "imu", default=(0, 0)),
            device=self.device,
        )
        ...
        mdp.reset_latency_buffers(self, torch.arange(self.num_envs, device=self.device))
    ...
    def _create_envs(self):
        ...
        target_feet_names = ["FL_foot", "FR_foot", "RL_foot", "RR_foot"]
        feet_names = [s for s in body_names if s in target_feet_names]
        ...
        joint_names = []
        for name in self.cfg.asset.joint_name:
            joint_names.extend([s for s in self.dof_names if name in s])
        print("###joints name:",joint_names)
        ...
        self.joint_indices = torch.zeros(len(joint_names), dtype=torch.long, device=self.device, requires_grad=False)
        for i in range(len(joint_names)):
            self.joint_indices[i] = self.gym.find_actor_dof_handle(self.envs[0], self.actor_handles[0], joint_names[i])
